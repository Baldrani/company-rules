...
program
    ...
    try {
        ...
        parseSpinner.succeed(chalk_1.default.green(`Parsed ${parsedInstructions.instructions.length} instructions`));
        if (options.verbose) {
            console.log(chalk_1.default.gray('Instructions:'), parsedInstructions.instructions.map(i => i.name).join(', '));
        }
        ...
        if (options.interactive) {
            ...
            const choices = await inquirer_1.default.prompt([
                ...
                },
                {
                    ...
                    choices: constants_1.SUPPORTED_IDES.map(ide => ({
                        ...
                        value: ide.value,
                        checked: constants_1.DEFAULT_SELECTED_IDES.includes(ide.value)
                    }))
                },
                {
                    type: 'checkbox',
                    ...
                    choices: constants_1.SUPPORTED_AGENTS.map(agent => ({
                        ...
                        value: agent.value,
                        checked: constants_1.DEFAULT_SELECTED_AGENTS.includes(agent.value)
                    }))
                },
                {
                    type: 'checkbox',
            ...
            if (choices.useLLM) {
                ...
                if (!existingKey) {
                    ...
                    const { provideKey } = await inquirer_1.default.prompt([
                        {
                            type: 'confirm',
                            name: 'provideKey',
                    ...
                    if (provideKey) {
                        ...
                        console.log(chalk_1.default.green(`✅ API key set for this session - LLM enhancement enabled\n`));
                        console.log(chalk_1.default.gray(`💡 To persist this key, add to your shell profile: export ${envVar}="your_api_key"`));
                    }
                    else {
                        ...
                        if (!continueWithoutLLM) {
                            console.log(chalk_1.default.cyan('💡 Run the command again to set up LLM enhancement.'));
                            process.exit(0);
                        }
                        else {
                            console.log(chalk_1.default.gray('   Continuing with basic rule generation...\n'));
                        }
                    }
                }
                else {
                    console.log(chalk_1.default.green(`✅ Found ${envVar} - LLM enhancement enabled\n`));
                }
            }
            selectedIdes = choices.selectedIdes || constants_1.DEFAULT_SELECTED_IDES;
            selectedAgents = choices.selectedAgents || constants_1.DEFAULT_SELECTED_AGENTS;
            selectedInstructions = parsedInstructions.instructions.filter(instruction => choices.selectedInstructions?.includes(instruction.name) ?? true);
        }
        else {
            // Use defaults when not in interactive mode
            selectedIdes = constants_1.DEFAULT_SELECTED_IDES;
            selectedAgents = constants_1.DEFAULT_SELECTED_AGENTS;
        ...
        if (!options.interactive) {
            if (llmService) {
                ...
                console.log(chalk_1.default.gray('   Or use --interactive to configure LLM enhancement'));
            }
        }
        // Generate files
...
program
    ...
    if (fs.existsSync('./instructions.yml')) {
        console.log(chalk_1.default.yellow('📝 instructions.yml already exists and will not be overwritten'));
        console.log(chalk_1.default.gray('💡 Check out'), chalk_1.default.cyan('examples/instructions.yml'), chalk_1.default.gray('for more rule ideas'));
        console.log(chalk_1.default.cyan('🚀 Run:'), chalk_1.default.white.bold('npx instructor install --interactive'), chalk_1.default.gray('to configure your setup'));
        return;
    }
    let config;
    if (options.interactive) {
    ...
    console.log(chalk_1.default.gray('📋 Rules defined:'), `${config.instructions.length} coding rules`);
...
program
    ...
    try {
        ...
        console.log(chalk_1.default.gray('💡 IDEs and agents are selected at runtime'));
        if (instructions.instructions.length > 0) {
            console.log(chalk_1.default.gray('🔧 Available rules:'), instructions.instructions.slice(0, 5).map(i => i.name).join(', ') + (instructions.instructions.length > 5 ? '...' : ''));
        }
        ...
        console.log('');
        console.log(chalk_1.default.bold('📄 Existing generated files:'));
        const possibleFiles = [
            'CLAUDE.md',
        ...
        if (existingFiles.length > 0) {
            ...
            console.log(chalk_1.default.gray('   (none found)'));
        }
        console.log('');
        console.log(chalk_1.default.cyan('💡 Run'), chalk_1.default.white.bold('instructor install --interactive'), chalk_1.default.cyan('to generate configuration files'));
    }
    catch (error) {
    ...
    try {
            ...
            if (fs.existsSync(dir)) {
                try {
                    ...
                    if (contents.length === 0) {
                        fs.rmdirSync(dir);
                        console.log(chalk_1.default.gray(`   ℹ️  Removed empty directory: ${dir}`));
                    }
                }
                catch (error) {
                    // Directory might not be empty or have permission issues
                    console.log(chalk_1.default.yellow(`   ⚠️  Could not remove directory ${dir}: ${error instanceof Error ? error.message : error}`));
                }
            }
        });
        ...
        if (fs.existsSync(gitignorePath)) {
            ...
            if (filteredLines.length !== lines.length) {
                ...
                console.log(chalk_1.default.gray(`   ℹ️  Cleaned generated entries from .gitignore`));
            }
        }
...
program
    ...
    console.log(chalk_1.default.gray('   ℹ️  instructions.yml will be preserved'));
    existingFiles.forEach(file => {
        console.log(chalk_1.default.red('   ✗'), file);
    });
...
program
    ...
    try {
        ...
        console.log(chalk_1.default.gray('Preview using defaults:'), constants_1.DEFAULT_SELECTED_IDES.join(', '), 'and', constants_1.DEFAULT_SELECTED_AGENTS.join(', '));
        console.log(chalk_1.default.gray('Instructions:'), `${parsedInstructions.instructions.length} rules`);
        console.log('');
        // Group by type
